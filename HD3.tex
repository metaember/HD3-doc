...
\begin{document}
	...
	***explanation to have some content, matrix multiplication of size $n*n$, $O(n^3)$ time complexity ...***
	Here is why matrix multiplication are the basic operations in machine learning, when using the basic feed forward structure in the back propagation algorithm:
	...
	***great explanations, we just have to explain the same thing here, put in some formulas in Latex showing that there is a lot of matrix multiplications>
	...
	Here, we merged the training set and test set to then be able to choose the number of images used in the training set and the testing set. Merging and spliting them as we wish allows us to have different proportion of input data in the training set and the test set.
	...
	Note that we don't know yet which distance to use. However, since we are working in finite dimension, all the norms are equivalent, and it is therefore convenient and not restricting to chose the Euclidian distance ($L^2$ norm), as one possible distance. Another distance that could be chosen is the angular distance.
